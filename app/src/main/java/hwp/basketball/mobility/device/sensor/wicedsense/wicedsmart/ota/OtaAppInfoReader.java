...
import hwp.basketball.mobility.device.sensor.wicedsense.util.ByteUtils;
import hwp.basketball.mobility.device.sensor.wicedsense.util.GattRequestManager;
import timber.log.Timber;


public class OtaAppInfoReader extends BluetoothGattCallback implements Handler.Callback,
        GattRequestManager.GattTimeoutCallback {
    ...
    @Override
    public void onTimeout(GattRequestManager.GattRequest request) {
        if (request == null) {
            return;
        }
        ...
        if (UUID_WS_SECURE_UPGRADE_CHARACTERISTIC_APP_INFO.equals(c.getUuid())) {
            Timber.w( "read(): error timeout reading characeristic...");
