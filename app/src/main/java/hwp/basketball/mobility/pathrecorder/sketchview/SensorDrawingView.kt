...
class SensorDrawingView : View, SensorDrawingViewViewContract.View {
     ...
     * */
    private val movingAverageLinearAcc = Array(3, { MovingAverage(30, 20) })
    ...
    private val accBasedMoves: ConcurrentLinkedQueue<Move> = ConcurrentLinkedQueue()
    ...
    private var actualEndPoint = PointF()
//    private var desiredEndPoint = PointF()

    /**Distance of the current location from the desired path.
     * Used for detecting when user is off the path*/
    private var currentDistanceFromDesiredPath: Double = 0.0
    /**
     * Path generated by using values from sensor.*/
    private var sensorPath: Path = Path()
    private var offDesiredPath: Path = Path()
    ...
    private val viewDimens: ViewDimens by lazy {
        ViewDimens(width, height)
    }

    private val pathViewPoints: List<PointF> by lazy {
        /**
    ...
    val pathSegmentsLengths: List<Float> by lazy {
        val list = mutableListOf<Float>()
        var len = getPathlengthForSegmentIndex(0)
        list.add(len)
        for (i in 1..pathViewPoints.size - 2) {
            len = getPathlengthForSegmentIndex(i)
            list.add(len + list[i - 1])
        }
        list
    }

    private fun getPathlengthForSegmentIndex(i: Int): Float {
        val path = Path()
        path.moveTo(pathViewPoints[i])
        path.lineTo(pathViewPoints[i + 1])
        return PathMeasure(path, false).length
    }

    private fun initBeforeDrillStart() {
    ...
    //path paint
    private var paintDesiredPath: Paint = Paint()
    private var paintSensorPath: Paint = Paint()
    private var paintCurrentLocation: Paint = Paint()
    private var paintArea: Paint = Paint()

    private lateinit var courtBackGround: Bitmap
...
//        bitmap.recycle()
    ...
    private lateinit var presenter: SensorDrawingViewViewContract.Presenter

    override fun setPresenter(sensorDrawingViewPresenter: SensorDrawingViewViewContract.Presenter) {
        this.presenter = sensorDrawingViewPresenter
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (ViewUtils.isInDebugMode) {
            presenter.displayStats(getStatistics())
        }
    ...
    private fun updateSensorPath() {

        currentSegmentIndex = updateCurrentSegmentIndex()

        if (currentSegmentIndex > pathSegmentsLengths.size - 1) {
            if (presenter.isInRecordingMode()) {
                presenter.onDrillCompleted()
            }
            return
        }
...
//                val angle = currentPathEnd.getAngleRad(currentDesiredPathEnd)
    ...
    private fun updateCurrentSegmentIndex(): Int {
        pathSegmentsLengths.forEachIndexed { index, length ->
            if (distanceCovered < length) {
                return index
            }
        }
        return pathSegmentsLengths.size
    }
...
//        pathDst.reset()
//        pathDst.moveTo(startPoint)

        initBeforeDrillStart()
    }

    private fun calculateLinearAccAndGravity(sample: BMSensorManager.AccData) {
        for (i in 0..2) {
    ...
    override fun addAccData(accData: BMSensorManager.AccData) {
        ...
        var squaresSumLinearAcc = 0.0
        for (i in 0..2) {
            val movingLinearAvgAccInG = linear_acceleration[i] / 1000f
            squaresSumLinearAcc += movingLinearAvgAccInG * movingLinearAvgAccInG
        }
